...
print("🔧 Сборка Minesweeper Game...")

# 1. Оптимизируем JS через uglify-js
print("📦 Оптимизация JS...")
subprocess.run([
    "npx", "uglifyjs", "minesweeper.js",
    "--compress", "drop_console=true,passes=2",
    "--mangle", "toplevel=true",
    "--mangle-props", "regex=/^_/",
    "--output", "minesweeper.min.js"
], check=True)

# Читаем минифицированный JS
with open('minesweeper.min.js') as f:
    js = f.read()

# 2. Минифицируем CSS
print("🎨 Минификация CSS...")
with open('minesweeper.css') as f:
    css = f.read()
...
os.remove('minesweeper.min.js')

# Считаем размеры
original_size = os.path.getsize('index.html') + os.path.getsize('minesweeper.css') + os.path.getsize('minesweeper.js')
